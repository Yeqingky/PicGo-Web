...
>
> **前端由两部分组成**（D94）：
> - **内置 SPA**（`go:embed web/dist`）—— 承载图库、上传、任务、日志、设置、后台、认证页
...
| **其余页面** | 全部是**内置 SPA**（embed）；图库/上传/任务/日志/设置/后台/认证页**不受主题影响** |
...
> ⚠️ **内置 SPA 不占用 `/`**：控制台概览页固定使用 **`/overview`**，避免与主题首页冲突。

### 3.1 公开（无需登录）

| 路由 | 页面 | 由谁渲染 | 说明 |
|---|---|---|---|
| `/` | 首页（落地页） | **当前主题**（默认主题注册的就是它） | Hero + 上传区 + 核心能力 + 应用场景 + FAQ + CTA + Footer。**内置 SPA 不占用该路径** |
...
| 路由 | 页面 | 由谁渲染 | 内容 |
|---|---|---|---|
| `/overview` | 概览 | 内置 SPA | 登录后的控制台落地页（**不是 `/`**：`/` 属于主题首页） |
| `/upload` | 上传 | 内置 SPA | 拖拽/点击/粘贴上传；目标存储选择；上传队列 |
| `/gallery` | 图库 | 内置 SPA | 瀑布流/网格；筛选；批量操作；**管理员在此页用顶部 Tab 切「我的/全部」**（D71） |
| `/gallery/:uid` | 图片详情 | 内置 SPA | 大图预览 + 元数据 + 外链复制 + 重命名/删除 |
...
| `/admin/logs` | 全部日志 | 内置 SPA（永久保留） | 仅管理员可见；全站操作日志 + 邮件日志 Tab |

> 路由守卫：`RequireAuth`（未登录 → `/login?redirect=`）、`RequireAdmin`（非 admin → 403 页）。
> 登录成功、已登录访问登录页、错误页「返回」一律回 **`/overview`**（**不是 `/`**，生产环境 `/` 是主题首页）。
...
- 布局：应用式外壳（根容器 `h-svh` + `overflow-hidden`，内层行 `min-h-0`），顶栏与侧栏**固定不随页面滚动**，内容区是唯一滚动容器；侧栏导航过长时在侧栏内部滚动（`.tsx` 里的 `overflow-y-auto`）。
- 顶栏：左侧汉堡（移动端）+ 站点名；右侧命令面板（`⌘K`）、语言切换、主题切换、用户菜单。语言切换位于主题切换左侧。
- 用户菜单：昵称 + 头像；含「个人设置 / 退出登录」。
- 侧栏底部参考 Immich 的状态区：显示存储空间卡片（含使用进度）、服务器状态圆点和当前版本号。
  SSE 断连状态通过圆点颜色表达，不在内容区顶部插入提示条; `CapacityBytes = 0` 时显示「不限额」。
  GitHub 最新正式 Release 高于当前版本时, 在版本号右侧右对齐显示「新版本!」链接; 无 Release、当前版本更高或检查失败时不显示。
  版本格式为 `x.x.x` 或 `x.x.x-fixN`, `fixN` 表示错误修复版本, 同一基础版本下修复编号越大版本越新。
...
> 概览页是**内置 SPA**（`src/features/overview/`），固定路径 **`/overview`**（D102）——
> **不占用 `/`**：`/` 是主题首页（D94）。登录成功、已登录访问 `/login`、403/404 的「返回」均指向这里。
>
> **数据来源**：`GET /api/web/v1/system/stats`（需登录）。后端按角色决定 scope：
> 普通用户只统计自己的上传；管理员额外拿到 `Users` 与 `ByStorage`。
> 配额（`CapacityBytes` / `UsedBytes`）取登录态里的 `User`，不额外发请求。
>
> **参考实现**：`lsky-pro` 的用户仪表盘（图片数量 / 可用储存 / 使用储存 / 总储存 四卡 + 我的信息 + 角色组信息）
> 与其**管理员控制台**（今日/昨日/本周/本月上传 + 近 30 天趋势图 + 系统情况），
> 以及 `skyImage` 的 Dashboard（容量上限 / 已用 / 剩余 + SVG 配额圆环）与 AdminConsole（用户数 / 文件数 / 存储用量）。
> 差异：本项目**不做**用户组/角色组配置展示（D53 无用户组）与系统环境信息（那是 `/admin/site → 关于` 的事）。
...
├──────────────────────────────────────────────────────────────┤
│ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐        │
│ │ 存储用量 │ │ 图片总数 │ │ 今日上传 │ │ 进行中任务│        │  ← 4 卡（sm 2 列 / xl 4 列）
│ │ 1.2 GB   │ │ 57       │ │ 11       │ │ 1        │        │
│ └──────────┘ └──────────┘ └──────────┘ └──────────┘        │
├───────────────────────────────┬──────────────────────────────┤
│ 存储配额                      │ 上传状态                     │
...
│   0 ┼─────────────────────────────────────────                │
│     08-17            08-31            09-15                 │
│     合计 188      单日峰值 24（2026-09-14）                  │
├───────────────────────────────┬──────────────────────────────┤
│ 站点概况（**仅管理员**）      │ 按存储分布（**仅管理员**）   │
│  用户总数/启用/禁用/管理员    │  GitHub · work  29  50.9%    │
│  [ 管理用户 → ]               │  WebDAV         28  49.1%    │
├───────────────────────────────┴──────────────────────────────┤
│ 我的信息  昵称 · 邮箱 · 角色 · 注册时间 · 最近登录 · 去设置  │
└──────────────────────────────────────────────────────────────┘
```
...
| 区块 | 组件 | 说明 |
|---|---|---|
| 资源卡片 | `components/stat-card.tsx` | 图标色用语义 token（不进新 token）；数值用 `tabular-nums` |
| 配额环 | `components/quota-donut.tsx` | 纯 SVG 圆环（`stroke-dasharray`）；`CapacityBytes = 0` → 中心显示「不限额」且不画弧 |
| 趋势图 | `components/trend-chart.tsx` | **不引入图表库**；柱体用 `viewBox=0 0 100 100` + `preserveAspectRatio=none` 只画 `fill`，网格线与文字用 HTML 覆盖层（避免非等比缩放把文字拉变形） |
| 存储分布 | `components/storage-breakdown.tsx` | `Progress` 条；`Name` 为空表示存储已删除 |

**三条硬约束**

1. **不引入图表库 / 组件库**（DESIGN.md §1）：趋势图、圆环、进度条全部用 SVG + Tailwind 自写。
2. **不显示缩略图**：概览页不拉图库列表（D84 的精神同样适用于这里——不为了好看而引入缩略图管线）。
3. **管理员区块只依据 `Role === 'admin'` 渲染**；数据侧由后端保证不给普通用户返回 `Users` / `ByStorage`（隐藏区块**不是权限**）。
...
- 点击行 → 详情抽屉（`Sheet`）：基本信息 + 逐行日志（自动滚底、可下载）+ 结果 JSON；长 JSON 展开按钮只显示 i18n 文案「展开全部 / Show all」, 不显示总行数。
- 进行中的任务进度条实时更新（SSE）；完成后状态徽章变化 + Toast 提示。
- 「清理已完成」批量删除（`DELETE /jobs/{uid}`）。

### 5.6 全部日志 `/admin/logs`（仅管理员）
...
- 该页面仅管理员可见, 普通用户没有入口且访问 `/logs` 不会渲染日志页面。
- 类型下拉来自 `GET /logs/types`（D45 的类型清单）；接口只返回稳定的 `Type` 值, 展示名称由前端 `src/i18n` 按当前语言映射。
- 关键词搜索匹配 `Username / TargetUID / Detail / Error`。
- 行首用状态色点（成功绿 / 失败红）；失败行默认展开错误摘要。
- 点行 → 详情抽屉：完整 JSON（`Detail` / `Error` 高亮格式化）。
- 页面提供「邮件日志」Tab（`EmailLogs`, **不显示正文**, D29）。
...
| `OperationLogTable` | 日志表 + 筛选 + 详情抽屉 | `/admin/logs` |
...
| `QuotaBar` | 配额进度卡片 | 侧栏底部 |
| `SchemaFormField` | 单字段渲染（按**内部类型**分派，§7.1；插件与主题 schema 共用） | 动态表单内部 |
| `ThemeToggle` / `LanguageToggle` / `CommandMenu` | 亮暗色切换 / 语言切换 / `⌘K` 命令面板（**注意：此「Theme」指亮暗色，与「页面主题」无关**） | 顶栏 |
...
| `←` `→` | 灯箱上一张 / 下一张 | 灯箱开启时 |
| `Esc` | 关闭浮层 / 清除选择 | 全局 |
| `g` 然后 `u` / `g` | 跳转 上传 / 图库 | 全局（不带输入焦点时） |

命令面板打开时不默认高亮第一项; 只有键盘上下键或鼠标移入后才显示选中态。搜索框与关闭按钮不显示圆角焦点框.

### 9.2 拖拽与粘贴

- 拖入窗口任意位置：显示整页虚线遮罩「松开即可上传」。
- 粘贴：读取 `ClipboardEvent.items` 里的图片；也支持从文件管理器复制**文件**后粘贴。
- 拖拽排序：Repeater 项排序用 `@dnd-kit`。
...
| 乐观更新 | 重命名等低风险操作先更新 UI 再请求，失败回滚 + Toast |
...
- 断线：指数退避重连（1s → 2s → 4s → 8s，上限 30s）；每次重连前先用 `GET /auth/me` 校验会话，让 HTTP 客户端在 `pcw_at` 过期时用 `pcw_rt` 静默刷新，期间由侧栏底部状态区显示「服务器离线」; 不打断内容区布局。
...
|---|---|---|---|---|
| 普通用户 | 仅「我的图片」（无 Tab） | 不可见 | 不可见 | 不可见 |
| 管理员 | 顶部 Tab：我的 / 全部 | 全部操作日志 + 邮件 | 可见 | 可见 |
...
- 中文为默认语言；内置 `zh-CN` 与 `en` 两个可切换 locale，`src/i18n/locales/zh-CN.json` 为源，`en.json` 提供英文文案（D75）。
- **文案禁止硬编码**，用 `t('KEY')`；key 用 `SCREAMING_SNAKE_CASE`；内置页面使用的 UI key 必须同时提供 `zh-CN` 与 `en` 文案，避免英文界面回退显示中文。
...
└── src/
    ...
    └── features/                  # 按业务域切分（与 PLAN.md W8 一致）
        ...
        ├── upload/ gallery/ jobs/ settings/ logs/
...
| # | 事项 | 最终结果 |
|---|---|---|
| 1 | **前端内置** | ✅ 内置 SPA 用 `go:embed web/dist`；图库/上传/任务/日志/设置/后台/认证页**全部内置**（D94） |
