...
| **Lsky API v1 兼容** | 完整实现 Lsky Pro v1 契约（9 个端点 + Lsky 信封 + Laravel 分页），**PicGo 桌面端 / PicList / uPic / ShareX 可直接把本站当图床**；启动时做路由冲突自检 |
...
> 两点说明：
> - 这一层是**外部冻结契约**，它的路径与字段名（snake_case）、时间格式（`Y-m-d H:i:s`）
>   与分页形状（Laravel 风格）**不会**跟随本项目的命名规范变化；
> - Lsky 层**只认 API Token**（不认浏览器登录态）。每次 `POST /api/v1/tokens`
>   会签发**新令牌并吊销同名的旧令牌** —— 即「重新登录 = 换一把新钥匙」。
