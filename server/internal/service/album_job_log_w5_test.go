...
func TestAlbumCreateAndNameConflict(t *testing.T) {
	e := newW5Env(t)
	alice := e.makeUser("usr_alice", "alice@example.com", model.UserRoleUser, nil)

	v, err := e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "壁纸", Intro: "桌面"},
		alice, "", "")
	wantOK(t, err)
	if v.UID == "" || v.Name != "壁纸" {
		t.Fatalf("创建结果不符：%+v", v)
	...
	// 同用户重名 → 40901
	_, err = e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "壁纸"}, alice, "", "")
	...
	// 另一个用户可以同名（唯一约束是 (UserUID, Name)）
	bob := e.makeUser("usr_bob", "bob@example.com", model.UserRoleUser, nil)
	_, err = e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "壁纸"}, bob, "", "")
	wantOK(t, err)

	// 空名拒绝
	_, err = e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "  "}, alice, "", "")
	wantCode(t, err, response.CodeInvalidParam)
}

// TestAlbumImageCountMaintained 断言冗余计数 `ImageCount` 被正确维护。
func TestAlbumImageCountMaintained(t *testing.T) {
	e := newW5Env(t)
	e.startQueue()
	...
	album, err := e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "合辑"}, alice, "", "")
	wantOK(t, err)

	a := e.uploadOne(alice, "a.png", 10, storage.UID)
	b := e.uploadOne(alice, "b.png", 10, storage.UID)

	// 移入
	res, err := e.albumSvc.MoveUploads(context.Background(), []string{a, b}, album.UID, alice, "", "")
	wantOK(t, err)
	if res.Moved != 2 {
		t.Fatalf("应移动 2 张，实际 %d", res.Moved)
	}

	got, err := e.albumSvc.Get(album.UID, alice)
	wantOK(t, err)
	if got.ImageCount != 2 {
		t.Fatalf("ImageCount 应为 2，实际 %d", got.ImageCount)
	}

	// 移出一张
	res, err = e.albumSvc.MoveUploads(context.Background(), []string{a}, "", alice, "", "")
	wantOK(t, err)
	if res.Moved != 1 {
		t.Fatalf("应移动 1 张，实际 %d", res.Moved)
	}

	got, err = e.albumSvc.Get(album.UID, alice)
	wantOK(t, err)
	if got.ImageCount != 1 {
		t.Fatalf("移出后 ImageCount 应为 1，实际 %d", got.ImageCount)
	}
}

// TestAlbumDeleteWithImagesRejected 断言有图片时删除相册 → 40901。
func TestAlbumDeleteWithImagesRejected(t *testing.T) {
	e := newW5Env(t)
	e.startQueue()
	...
	album, err := e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "有图"}, alice, "", "")
	wantOK(t, err)
	uid := e.uploadOne(alice, "a.png", 10, storage.UID)
	_, err = e.albumSvc.MoveUploads(context.Background(), []string{uid}, album.UID, alice, "", "")
	wantOK(t, err)

	// 默认拒绝
	_, err = e.albumSvc.Delete(context.Background(), album.UID, false, alice, "", "")
	wantCode(t, err, response.CodeConflict)

	// WithUploads=true → 只脱离，不删图片
	res, err := e.albumSvc.Delete(context.Background(), album.UID, true, alice, "", "")
	wantOK(t, err)
	if !res.Deleted || res.DetachedUploads != 1 {
		t.Fatalf("应脱离 1 张并删除相册：%+v", res)
	}

	// **图片仍在**（硬删除只针对图片本身，不随相册删除）
	up, err := e.gallery.Get(uid, alice)
	wantOK(t, err)
	if up.AlbumUID != "" {
		t.Fatalf("图片应已脱离相册，实际 AlbumUID=%s", up.AlbumUID)
	}
}

func TestAlbumCoverMustBelongToOwner(t *testing.T) {
	e := newW5Env(t)
	e.startQueue()
	...
	album, err := e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "封面测试"}, alice, "", "")
	wantOK(t, err)

	bobImg := e.uploadOne(bob, "bob.png", 10, storage.UID)
	_, err = e.albumSvc.Update(context.Background(), album.UID,
		UpdateAlbumInput{CoverUploadUID: &bobImg}, alice, "", "")
	wantCode(t, err, response.CodeInvalidParam)

	aliceImg := e.uploadOne(alice, "alice.png", 10, storage.UID)
	v, err := e.albumSvc.Update(context.Background(), album.UID,
		UpdateAlbumInput{CoverUploadUID: &aliceImg}, alice, "", "")
	wantOK(t, err)
	if v.CoverURL == "" {
		t.Fatal("CoverURL 应被解析出来（便于前端直接展示）")
	}
}

// TestAlbumCrossUserAccessDenied 断言不能操作他人相册。
func TestAlbumCrossUserAccessDenied(t *testing.T) {
	e := newW5Env(t)

	alice := e.makeUser("usr_alice", "alice@example.com", model.UserRoleUser, nil)
	bob := e.makeUser("usr_bob", "bob@example.com", model.UserRoleUser, nil)

	album, err := e.albumSvc.Create(context.Background(), CreateAlbumInput{Name: "私有"}, alice, "", "")
	wantOK(t, err)

	_, err = e.albumSvc.Get(album.UID, bob)
	wantCode(t, err, response.CodeForbidden)

	newName := "改名"
	_, err = e.albumSvc.Update(context.Background(), album.UID, UpdateAlbumInput{Name: &newName}, bob, "", "")
	wantCode(t, err, response.CodeForbidden)

	_, err = e.albumSvc.Delete(context.Background(), album.UID, true, bob, "", "")
	wantCode(t, err, response.CodeForbidden)
}
